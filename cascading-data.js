...
function parseCascadingData() {
    ...
    lines.forEach(line => {
        ...
        if (parts.length === 2) {
            ...
            // Chiefdom||PHU||UID format
            const [chiefdom, phu, uid] = parts.map(s => s.trim());
            
            if (!chiefdomPHUMap[chiefdom]) {
                chiefdomPHUMap[chiefdom] = [];
            }
            if (!chiefdomPHUMap[chiefdom].includes(phu)) {
    ...
    return { regionDistrictMap, districtChiefdomMap, chiefdomPHUMap, phuUIDMap };
}

// Export for use in main file
if (typeof module !== 'undefined' && module.exports) {
    module.exports = { CASCADING_DATA, parseCascadingData };
}
